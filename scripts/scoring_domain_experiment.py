...
def build_base(
    ...
    base = pd.concat(parts).sample(frac=1.0, random_state=random_state).reset_index(drop=True)
    return base
...
def _thin_positives(frame: pd.DataFrame, target_rate: float, random_state: int) -> pd.DataFrame:
    ...
    demoted = positives.drop(
        positives.sample(n=keep, random_state=random_state).index
    ).assign(**{LABEL_COL: 0})
    return pd.concat([positives.sample(n=keep, random_state=random_state), demoted, negatives])
...
def _table(frame: pd.DataFrame, decimals: int = 4) -> str:
